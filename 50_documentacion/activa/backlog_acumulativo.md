...
### Sesión 30 — 2026-09-23

175. [DOC] Corrección de `commit_cierre` en `ESTADO.md`, que apuntaba a `3b17b9b` cuando el cierre real de la sesión 29 fue `f7279eb`, más `ultima_actividad` y la marca `sesion_abierta: true`. Primer acto de la sesión, instruido por el traspaso v29. Commit delegado: el puente de Cowork no tiene identidad de Git ni credenciales.
176. [DOC] Plan de la vista de trayectorias (`20260909_plan_spinoff_trayectoria_traspasos.md`): cuatro decisiones de diseño cerradas (eje en años calendario con el traspaso marcado, referente municipal no traspasado en el mismo plano, los 36 Servicios Locales con marca de madurez, cero dependencias de red) sobre una base medida en los parquet intermedios (36 Servicios con 2.337 establecimientos, nueve años con medición, hueco 2019-2021, seis cohortes, 11 de 36 con cuatro o más años posteriores a su traspaso). El plan proponía repositorio aparte; el titular lo descartó al cierre (entrada 194).
177. [UI] Construcción de la vista de trayectorias como archivo HTML autocontenido: SVG y JavaScript escritos a mano, sin librerías, sin CDN, y las tres caras de gobCL incrustadas como `data:` URI. Se trabajó en el chat después de que un intento de delegar el diseño a Claude Design resultara inservible. Nace resuelta en lo que el motor sigue debiendo desde la entrada 166.
178. [UI] Selector de cohorte con estelas: al elegir una cohorte se dibuja solo esa cohorte. Tres botones de velocidad y eliminación de la codificación por color de madurez, que agregaba ruido sin haberse pedido.
179. [DOC] Encargo de diseño `20260911_brief_diseno_spinoff.md`, corregido dos veces dentro de la sesión al descubrirse que dos de sus premisas eran falsas: la cadena de pérdida de filas no era aditiva y el denominador de la cobertura no existía.
180. [D] Diagnóstico de filas anómalas del insumo en `20260911_filas_anomalas_simce_rbd.xlsx`, tres hojas: resumen, 1.524 filas sin `nalu` y 1.073 con `nalu` en cero, sobre columnas que el insumo declara obligatorias. Sin fórmulas, porque LibreOffice agotó el tiempo de recálculo de forma repetida; los conteos van como valores documentados con una nota en celda que explica por qué.
181. [UI] Tarjeta lateral de la vista: casilla por entidad con su número, el conteo de establecimientos que sostienen cada cifra y el desglose por grupo socioeconómico plegado por omisión. La cobertura se declara en pantalla, no en una nota al pie.
182. [UI] Panel de serie completa como control de rigor: interruptor que restringe a los establecimientos presentes en todos los años, para separar cambio de composición de cambio de resultado. Retiro del año preliminar de la vista.
183. [UI] Nube de contexto de 180 sostenedores municipales como puntos semitransparentes sin estela, con interruptor propio; con la nube encendida los dos ejes quedan fijos de 0 a 100 por ciento, porque el sistema de cuadrantes solo tiene sentido sobre escala completa.
184. [UI] Referente de sostenedores municipales no traspasados como círculo gris punteado, agregado congelado de esa misma nube, con su marca fuera de la escala de área y eso declarado. Se había eliminado por una lectura equivocada de una instrucción del titular y se restituyó; la primera restitución, una cruz de ejes, fue rechazada.
185. [UI] Retiro del margen de error binomial y del interruptor de descuento de la tendencia nacional, dos controles construidos y eliminados enteros: el primero porque Simce es censo de quienes rindieron y no muestra, el segundo porque nunca estuvo entre las sugerencias aprobadas.
186. [UI] Disciplina tipográfica y de color: tres tamaños de letra (21, 15 y 13 píxeles) más el año en 42, gobCL incrustada en sus tres caras, y la línea de tiempo en azul institucional oscuro en lugar de naranjo.
187. [UI] Burbujas legibles: radio mínimo de 12 píxeles para que el número quepa adentro, techo de 46, y orden de dibujo que pone las chicas sobre las grandes.
188. [UI] Modo presentación con el control de salida movido desde su posición flotante a la barra del reproductor, como icono de 40×40; medido en modo presentación: borde derecho a ras del panel del gráfico, centrado vertical exacto, 17 píxeles de margen interior, y oculto fuera del modo.
189. [P] Batería de verificación `verificar_trayectorias.R` sobre los parquet de `40_salidas/intermedios/`: ocho pruebas (D1 a D8) con `|>`, `.by=` y `here::here()`, salida a CSV en `logs/` y código de salida 1 ante fallo. D8 es el control positivo: altera una cifra en 0,5 puntos y exige exactamente un hallazgo. Es el primer verificador del proyecto que nace junto con lo que verifica y no después.
190. [DOC] Auditoría de la vista de trayectorias: 28 pruebas en cuatro familias (capa de datos, invariantes del motor, coherencia entre tabla y gráfico, presentación). Resultado tras las correcciones: 28 pasan, 0 fallan. Dos de las pruebas resultaron defectuosas y se acotaron antes de tocar el producto: una contaba los rótulos de los ejes como números fuera de burbuja, y otra contaba el número escalado por geometría como un quinto tamaño tipográfico.
191. [UI] Dos correcciones que destapó la auditoría: una burbuja desmarcada seguía respondiendo al cursor, porque `display:none` oculta pero no anula el radio; y el tooltip atribuía al año en curso cifras que venían de la última medición disponible, detectado por el titular en Huasco.
192. [D] Hallazgo sobre la asignación del grupo socioeconómico: la Agencia lo asigna por nivel y no por establecimiento, de modo que 6.461 de 53.329 pares escuela-año pertenecen a grupos distintos en 4° básico y en 2° medio. Consecuencia para todo el proyecto: cualquier vista que combine niveles y segmente por grupo no cuadra con su total, y se declara en vez de forzarse.
193. [DOC] Instrucción de apertura `20260917_instruccion_spinoff_trayectorias.md` para replicar la vista en los proyectos de asistencia y de matrícula, que sí son repositorios distintos: sección común (qué se copia tal cual, las seis decisiones de dominio a rehacer, los invariantes, el orden de trabajo) más una variante por dominio. Los nombres de los repositorios de destino van declarados como hipótesis, porque no se pudieron verificar desde esta sesión.
194. [REPO] Decisión del titular al cierre: la vista de trayectorias es parte de este proyecto y no un repositorio aparte, porque un repositorio propio habría duplicado la capa de datos, el escáner, el protocolo de cierre y la gobernanza para consumir exactamente los mismos insumos. En consecuencia queda pendiente sacarla de `50_documentacion/andamios/`, que la política §1.2 congela, y llevarla al patrón que ya usa el motor (plantilla más script generador en `30_procesamiento/`, salida publicable en `docs/`), sin integrarla como pestaña de `33_motor_template.html`, que ya tiene 4.582 líneas y depende de red mientras esta vista no.

**Delta del backlog:** 20 entradas nuevas (175–194). Sin cambios de taxonomía. Total acumulado: 194. El proyecto gana una tercera vista sin tocar el motor ni el pipeline, lo que explica que el delta sea casi todo interfaz y que la categoría de datos, la más pequeña del proyecto, duplique su población histórica: construir un instrumento nuevo obligó a mirar el insumo con más cuidado que consumirlo durante veintinueve sesiones. Es también la primera sesión en que una auditoría propia, y no el titular, encuentra defectos del producto, y la primera en que dos de las propias pruebas de auditoría resultan ser el defecto. La desviación de taxonomía que la entrada 157 dejó declarada empeora con este delta: interfaz sigue muy por encima del umbral de subdivisión, y sigue sin ajustarse porque reclasificar choca con el invariante append-only y es decisión del titular.

### Sesión 31 — 2026-09-23

195. [REPO] Apertura de emergencia: el cierre v30 no había corrido (paquete sin versionar, `ESTADO.md` en el índice, `.git/index.lock` huérfano del 2026-09-09). Se corrigió el paquete (`settings_version` a la v38, `patron` de ERR-30-02, autorización del xlsx de filas anómalas en `50_datos_versionados_autorizados.md`), se restauró `ESTADO.md`, se retiraron los candados a `_archivo/` y el cierre v30 corrió (`24cff4e`).
196. [DOC] Regla 12 y A29-4 en `50_diseno_ramas_deteccion.md`: el alcance del instrumento iguala el alcance de la afirmación, con su línea de comprobación. Resuelve el pendiente 5 de v29. Commit `2ab9927`.
197. [UI] Retiro de `unpkg.com`: React, ReactDOM y Babel standalone vendorizados en `10_utils/` con sha384 verificado, y el JSX de la plantilla transpilado en el build con Babel dentro de V8 (opción C, decisión `20260923_decision_transpilacion_en_build.md`). El motor abre sin red y el titular sigue editando JSX. Commits `65e5e4a` y `02daaf7`, publicado en `232960c`. Resuelve el pendiente 1 de v29.
198. [D] Diagnóstico del grupo 5 en la vista de trayectorias: el script de ensamblado descartó el grupo antes de agregar, así que el total `T` de la nube y del referente lo excluye (Las Condes, 4° básico Lectura 2023: 271 contra 344). En el catálogo SLEP, el grupo 5 cae completo por el filtro de supresión. Corrección pendiente en el traslado.
199. [D] Bases Simce 2025 finales (v22025, 2026-06-22) en lugar de las preliminares: comparación celda a celda sin ninguna diferencia en las 40 columnas de datos; las preliminares se movieron a `_archivo/20260923/` fuera de git, con md5 verificado. Commit `9bad097`.
200. [UI] El año preliminar se deriva de los insumos y deja de estar fijo en 2025; la leyenda del mapa de calor y la nota metodológica aparecen solo si hay algún año preliminar. 2025 queda sin asteriscos y con cifras idénticas. Commit `44dfb07`, publicado en `7e7b294`.
...
### Sesión 33 — 2026-09-24

206. [D] Regla de filas de la vista de trayectorias alineada con la del motor (D33-1): se excluyen las filas con marca de la Agencia y las de menos de 10 evaluados. En la base de la vista el umbral no descarta ninguna fila; las marcadas son 2.008 en el país y 610 en los Servicios Locales (2,1% de sus evaluados), y el referente pasa de 1.333 a 1.299 establecimientos. Las cifras de las notas metodológicas pasan a calcularse desde los datos (`cifras_notas()`, D33-6). Batería de 17 a 19 pruebas. Commit `498981b`. Resuelve el pendiente 4 de v32.
207. [UI] La vista de trayectorias adopta la identidad visual del motor (D33-3): encabezado y menú de vistas compartidos, con «Trayectorias de los Servicios Locales» como tercera entrada; paleta, componentes y fuente del sistema del motor; sin tema oscuro ni gobCL incrustada. Commit `3de223d`.
208. [UI] Corregidos los nueve defectos de forma heredados del mockup y la leyenda del referente (errata, coma decimal, aviso de grupo sin datos, cifras superpuestas y burbujas fuera del plano, referente tapado, contraste, texto del SVG que escalaba con el ancho, tabla lateral, notas), con mediciones antes y después en Chromium sin red. Commit `1d6c4ce`. Resuelve el pendiente 1 de v32.
209. [UI] Los menús de grupo socioeconómico y de nivel desactivan, con «(sin datos)», las opciones sin datos para la cohorte y la cobertura elegidas, y vuelven a una opción con datos cuando la elegida se vacía; ninguna combinación elegible deja el plano vacío. Commit `1d6c4ce`.
